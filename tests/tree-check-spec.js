...
$('<div class="row"><div class="span8" id="t3"></div><div class="span8" id="t31"></div><div class="span8" id="t32"></div></div>').prependTo('body');

describe('勾选',function () {
  var nodes = [
  ...
  describe('勾选的tree',function(){
    describe('初始化勾选',function(){
      ...
      it('测试是否可以勾选',function(){
        ...
        expect(tree.isCheckable(node)).to.be(false);
        expect($(element).find('.x-tree-icon-checkbox').length).to.be(0);
        var node = tree.getItem('3'),
          element = tree.findElement(node);
        expect(tree.isCheckable(node)).to.be(true);
        expect($(element).find('.x-tree-icon-checkbox').length).to.be(1);
      });
      it('默认勾选的文件夹',function(){
        var node = tree.getItem('21');
        expect(tree.isChecked(node)).to.be(true);
        BUI.each(node.children,function(subNode){
          expect(subNode.checked).to.be(true);
        });
      });
      it('默认部分勾选文件夹',function(){
        var node = tree.getItem('2');
        expect(tree.hasStatus(node,'partial-checked')).to.be(true);
      });
      it('默认勾选叶子节点',function(){
         var node = tree.getItem('4'),
          element = tree.findElement(node);
        expect(tree.isCheckable(node)).to.be(true);
        expect(tree.hasStatus(node,'checked')).to.be(true);
        expect($(element).find('.x-tree-icon-checkbox').length).to.be(1);
      });
      it('默认勾选所有叶子节点',function(){
         var node = tree.getItem('1');
         expect(tree.isChecked(node)).to.be(true);
         expect(tree.hasStatus(node,'partial-checked')).to.be(false);
      });
    });
    describe('测试勾中的值',function(){
      ...
      it('获取全部勾选的值',function(){
        ...
        expect(BUI.Array.contains(node,nodes)).to.be(true);
      ...
      it('获取树节点下勾选的值',function(){
        ...
        expect(nodes.length).not.to.be(0);
      ...
      it('获取勾选的所有叶子节点',function(){
        ...
        BUI.each(nodes,function(subNode){
          expect(subNode.leaf).to.be(true);
          expect(subNode.checked).to.be(true);
        });
      });
    });
    ...
    describe('勾选操作',function(){
      it('勾选子节点',function(){
        ...
        expect(tree.hasStatus(node,'checked')).to.be(true);
        expect(tree.hasStatus(node.parent,'checked')).to.be(true);
        expect(tree.hasStatus(node.parent,'partial-checked')).to.be(false);
      });

      it('取消勾选子节点',function(){
        var node = tree.getItem('22');
        tree.setNodeChecked(node,false);
        expect(tree.hasStatus(node,'checked')).to.be(false);
        expect(tree.hasStatus(node.parent,'checked')).to.be(false);
        expect(tree.hasStatus(node.parent,'partial-checked')).to.be(true);
      });
      ...
      it('树节点勾中状态下，取消叶子节点勾中',function(){
        ...
        expect(tree.hasStatus(subNode,'checked')).to.be(true);
        tree.setNodeChecked(subNode,false);
        expect(tree.hasStatus(subNode,'checked')).to.be(false);
        expect(tree.hasStatus(node,'checked')).to.be(false);
        expect(tree.hasStatus(node,'partial-checked')).to.be(true);
      });
      ...
      it('勾选所有叶子节点',function(){
        ...
        expect(tree.hasStatus(tree.getItem('21'),'checked')).to.be(false);
        expect(tree.hasStatus(tree.getItem('22'),'checked')).to.be(false);
        expect(tree.hasStatus(node,'partial-checked')).to.be(false);
      ...
      it('递归测试上级节点的半选状态',function(){
        ...
        tree.setNodeChecked(node,false);
        expect(tree.hasStatus(node.parent,'partial-checked')).to.be(true);
        expect(tree.hasStatus(node.parent.parent,'partial-checked')).to.be(true);

        tree.setNodeChecked(node,true);
        expect(tree.hasStatus(node.parent,'partial-checked')).to.be(false);
        expect(tree.hasStatus(node.parent.parent,'partial-checked')).to.be(false);
      });
      ...
      it('清除选中父节点',function () {
        ...
        tree.setNodeChecked(node,false);
        expect(tree.isChecked('21')).to.be(false);
        expect(tree.isChecked('211')).to.be(false);
        expect(tree.isChecked('22')).to.be(false);
      });
      ...
      it('设置孙子节点',function() {
        ...
        expect($(element).hasClass('bui-tree-item-partial-checked')).to.be(true);
...
describe('数据',function () {
  ...
  describe('勾选状态下的，增删改',function(){
    ...
    it('未勾选的树节点下，添加未勾选的字节点',function(){
      ...
      expect(subNode.checked).not.to.be(true);
      tree.expandNode(node);
      expect(tree.hasStatus(subNode,'checked')).to.be(false);
    });
...
describe('勾选模式',function () {
  ...
  describe('测试勾选模式',function(){
    ...
    it('测试勾选字段',function(){
      expect(tree.get('checkedField')).to.be('checked');
    });

    it('测试禁止所有勾选',function(){
      BUI.each(tree.get('root').children,function(subNode){
        expect(tree.isCheckable(subNode)).to.be(false);
        //expect(tree.hasStatus(subNode,'checked')).to.be(false);
      });
    });
    ...
    it('测试全部允许勾选',function(){
      ...
      BUI.each(tree.get('root').children,function(subNode){
        expect(tree.isCheckable(subNode)).to.be(true);
        //expect(tree.hasStatus(subNode,'checked')).to.be(subNode.checked);
      });
    });
    ...
    it('测试自由勾选',function(){
      ...
      tree.set('nodes',BUI.cloneObject(nodes));
      BUI.each(tree.get('root').children,function(subNode){
        expect(tree.isCheckable(subNode)).to.be(subNode.checked != null);
      });
    });
    ...
    it('测试仅叶子节点勾选',function(){
      ...
      tree.set('nodes',BUI.cloneObject(nodes));
      BUI.each(tree.get('root').children,function(subNode){
        expect(tree.isCheckable(subNode)).to.be(subNode.leaf);
    ...
    it('禁用节点',function(){
      ...
      expect(node.checkable).to.be(false);
      tree.expandNode('1');
...
describe('测试非级联选择',function () {
  ...
  describe('测试非级联选择',function(){
    ...
    it('测试默认勾选',function () {
      ...
      expect(tree.findNode('11').checked).to.be(true);
    ...
    it('测试子节点不默认勾选',function () {
      ...
      expect(tree.findNode('1').checked).to.be(false);
    });
